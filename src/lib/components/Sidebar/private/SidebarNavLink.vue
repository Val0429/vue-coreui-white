<template>
  ...
  >
    ...
    <b-badge
      v-if="data.badge && data.badge.label"
      pill
      :variant="data.badge.variant"
      >{{ data.badge.label }}</b-badge
    >
  </element>
</template>
...
import { lang } from "@root/core/i18n";
import { FindRouter } from "@root/core/router";
...
export class SidebarNavLink extends Vue {
  ...
  private created() {
    ...
    SidebarNavLink.subscription = lang.getObservable().subscribe((value) => {
      document.documentElement.style.setProperty(
        "--permission-denied",
        `"${lang.translate("mb_PermissionDenied")}"`
      );
      document.documentElement.style.setProperty(
        "--content-invalid",
        `"${lang.translate("mb_ContentInvalid")}"`
      );
    });
  }
  private destroyed() {
